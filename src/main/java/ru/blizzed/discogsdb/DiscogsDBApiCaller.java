...
public interface DiscogsDBApiCaller {
    ...
    /**
     * The Release Rating endpoint retrieves, updates, or deletes the rating of
     * a release for a given user.
     */
     ...
     * location, or other entity involved with Artists and Releases. Labels were
     * recently expanded in scope to include things that aren’t labels – the
     * name is an artifact of this history.
     */
    @GET("labels/{label_id}")
    ...
    Call<CollectionValue> getCollectionValue(@Query("key") String key, @Query("secret") String secret, @Path("username") String username);

    @GET("/users/{username}/collection/fields")
    Call<Fields> getListCustomFields(@Query("key") String key, @Query("secret") String secret, @Path("username") String username);

    // Wantlist
    @GET("users/{username}/wants")
    Call<Page<ReleaseSearchResult>> getWantlist(@Path("username") String username, @QueryMap Map<String, String> queryMap);
}
